...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  // Isolate from the workspace (src/lib/og/site.json, public/og.jpg) so the
  // document title wins, as it does for a fresh template workspace.
  const cwd = mkdtempSync(join(tmpdir(), "grok-og-chrome-"));
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd, site: {} });
...
test("injects into documents with no head element", () => {
  const cwd = mkdtempSync(join(tmpdir(), "grok-og-nohead-"));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const cwd = mkdtempSync(join(tmpdir(), "grok-og-stream-"));
  const injector = createHeadInjector({ appName: "Wild Race", cwd, site: {} });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const cwd = mkdtempSync(join(tmpdir(), "grok-og-title-"));
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Wild Race",
    cwd,
    site: {},
...
// GitHub Pages static builds set `serverDir: false`; Vercel builds must still
// use `"./server"`, so accept `serverDir: githubPages ? false : "./server"`.
test("vite config keeps the nitro serverDir wiring", () => {
  const viteConfig = readFileSync(join(TEMPLATE_ROOT, "vite.config.ts"), "utf8");
  assert.match(viteConfig, /serverDir:\s*(githubPages\s*\?\s*false\s*:\s*)?"\.\/server"/);
